...
function getNextFriday(date) {
  const daysToAdd = (12 - date.getDay()) % 7;
  return new Date(
    new Date(date).setDate(date.getDate() + (daysToAdd === 0 ? 7 : daysToAdd))
  );
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const workDays = [];
  const start = stringToDate(period.start);
  const end = stringToDate(period.end);
  let currWorkDaysCounter = 0;
  let currOffDaysCounter = 0;
  let currDate = new Date(start);

  while (currDate <= end) {
    if (!countOffDays || currWorkDaysCounter < countWorkDays) {
      currWorkDaysCounter += 1;
      workDays.push(dateToString(currDate));
    } else {
      currOffDaysCounter += 1;
      if (currOffDaysCounter === countOffDays) {
        currOffDaysCounter = 0;
        currWorkDaysCounter = 0;
      }
    }
